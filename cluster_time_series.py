...
def ToTreeJSON(clusterInfo, threshold, time_index, delay):
    clusters = clusterInfo['clusters']
    W = clusterInfo['W']
    levels = clusterInfo['levels']
    nodes = []
    ...
    for i in range(len(W)):
        childrenNames = clusters[str(i)]['members']
        children = []
        for j in range(len(childrenNames)):
            if i != childrenNames[j]:
                if(nodes[j]['children'] != []):
                    children.append({'name': j, 'children':nodes[j]['children']})
                else:
                    children.append({'name': j})
                
            
        
        nodes.append({'name': i, 'children': children})
    

    filename = str(time_index) + '_tree.json'
    fp = open(filename, 'w')
    data = json.dumps(nodes[len(W)-1])
...
if __name__ == "__main__":
  ...
  #      PlotClusteredMatrix(clusterInfo['clusters'], filename, start, end)
    ...
    for i in range(len(AtClusters)):
        ...
        ToTreeJSON(AtClusters[i], -1, i, delay)
